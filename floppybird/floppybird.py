...
def welcomeScreen():
    """
    Shows the welcome screen with the bird, background, and text.
    """
...
def mainGame():
    ...
    while True:
        ...
        # Add new pipe when the first pipe is about to cross the leftmost part of the screen
        if 0 < upperPipes[0]['x'] < 5:
            newPipe = getRandomPipe()
            upperPipes.append(newPipe[0])
            lowerPipes.append(newPipe[1])

        # Remove pipes if they are out of screen
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption('Flappy Bird by Your Name')
    ...
    GAME_SOUNDS['hit'] = pygame.mixer.Sound('gallery/sounds/hit.ogg')
    ...
    while True:
        welcomeScreen()#it will show the wellcome screen
        mainGame()